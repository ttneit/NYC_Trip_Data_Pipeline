...
def preprocess(initial_df) : 
    initial_df = initial_df.drop(*['trip_id'])
    ...
    preprocess_df = preprocess_df.withColumn('tpep_pickup_datetime',sf.to_timestamp(sf.col('tpep_pickup_datetime')))
    preprocess_df = preprocess_df.withColumn('tpep_dropoff_datetime',sf.to_timestamp(sf.col('tpep_dropoff_datetime')))
    preprocess_df = preprocess_df.withColumn('tpep_pickup_datetime', sf.date_format(sf.col('tpep_pickup_datetime'), "yyyy-MM-dd HH:mm:ss"))
    preprocess_df = preprocess_df.withColumn('tpep_dropoff_datetime', sf.date_format(sf.col('tpep_dropoff_datetime'), "yyyy-MM-dd HH:mm:ss"))
...
def ETL_yellow(preprocess_df,flag_df,date_df) : 
    df = preprocess_df.withColumn('Year',sf.year(sf.col('tpep_dropoff_datetime')))
    df = df.withColumn('Month',sf.month(sf.col('tpep_dropoff_datetime')))
    df = df.withColumn('Day',sf.dayofmonth(sf.col('tpep_dropoff_datetime')))
    df = df.withColumn('Hour',sf.hour(sf.col('tpep_dropoff_datetime')))
    ...
    new_df = new_df.withColumn('Year',sf.year(sf.col('tpep_dropoff_datetime')))
    new_df = new_df.withColumn('Month',sf.month(sf.col('tpep_dropoff_datetime')))
    new_df = new_df.withColumn('Day',sf.dayofmonth(sf.col('tpep_dropoff_datetime')))
    new_df = new_df.withColumn('Hour',sf.hour(sf.col('tpep_dropoff_datetime')))
    ...
    new_df = new_df.withColumn('service_type',sf.lit('yellow'))
    new_df = new_df.withColumn('trip_type',sf.lit(0))
    new_df = new_df.withColumnsRenamed({'tpep_pickup_datetime' : 'pickup_datetime','tpep_dropoff_datetime' : 'dropoff_datetime','payment_type' : 'PaymentTypeID','trip_type':'TripTypeID'})
...
def get_latest_raw_db(spark , table,keyspace ) :
    ...
    latest_time_row = data.select('tpep_pickup_datetime').agg({'tpep_pickup_datetime':'max'}).collect()
...
def get_latest_time_dw(spark) : 
    ...
    info_df = info_df.filter(sf.col('service_type') == 'yellow')
...
if __name__ == '__main__' : 
    ...
    spark = SparkSession.builder \
            ...
            .getOrCreate()
    latest_raw_time = get_latest_raw_db(spark,'yellow_taxi','nyc_taxi')
    ...
    if latest_raw_time > latest_preprocess_time :
        print("-------------------------------------------------------------------------------------------------------------------")
        print("Extract data ")
        initial_df = spark.read.format('org.apache.spark.sql.cassandra').options(table='yellow_taxi',keyspace='nyc_taxi').load()
        initial_df.show()

        print("-------------------------------------------------------------------------------------------------------------------")
        print("Filter the latest data ")
        initial_df = initial_df.filter(sf.col('tpep_pickup_datetime') > latest_preprocess_time)
        ...
        print("-------------------------------------------------------------------------------------------------------------------")
        print("Process data ")
        final_df = ETL_yellow(preprocess_df,flag_df,date_df)
        final_df.show()
        print("-------------------------------------------------------------------------------------------------------------------")
        print("Writing data ")
